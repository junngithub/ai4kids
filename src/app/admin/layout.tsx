...
export default async function AdminLayout({
  ...
  const hmacAdmin = verifyAdminSessionValue(cookieStore.get(ADMIN_COOKIE_NAME)?.value);
  const [brand, jwtSession] = await Promise.all([getSiteBrand(), getAdminSession()]);

  if (!hmacAdmin && !jwtSession) {
    redirect(`/admin/login?from=${encodeURIComponent(pathname || "/admin")}`);
  }

  // HMAC cookie ⇒ full admin; otherwise use the (staff-only) JWT role.
  const role: AdminRole = hmacAdmin ? "admin" : jwtSession?.role ?? "admin";
  const email = hmacAdmin?.email ?? jwtSession?.email ?? "";
