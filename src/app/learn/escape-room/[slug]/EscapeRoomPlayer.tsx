...
type Phase = "intro" | "explore" | "escaped";
...
export function EscapeRoomPlayer({ room }: { room: EscapeRoom }) {
  ...
  // Scene interaction. Character position is x/y (% of the scene) so it can
  // walk diagonally toward whichever object it's heading for.
  ...
  // Per-open-puzzle state (reset whenever a different station opens).
  ...
  const maxPoints = total * POINTS_FIRST_TRY;
  ...
  function visit(station: Station) {
    ...
    // Stand just below the object so the character doesn't cover it.
  ...
  function handleSolved() {
    ...
    setJustSolved(true);
    setPoints((p) => p + (wrongCount === 0 && !hintShown ? POINTS_FIRST_TRY : POINTS_WITH_HELP));
    setSolvedIds((s) => (s.includes(openStation.id) ? s : [...s, openStation.id]));
  }
  ...
  function tryDoor() {
    ...
    walkTo(82, 70);
    window.setTimeout(() => setPhase("escaped"), WALK_MS + 150);
  }
  ...
  // Save the score once, when the learner escapes.
  useEffect(() => {
    ...
    fetch("/api/learn/score", {
      ...
      body: JSON.stringify({
        ...
        score,
        metadata: { room: room.slug, stations: total },
      }),
    })
      .catch(() => {})
      .finally(() => {
  ...
  }, [phase, points, maxPoints, room.activitySlug, room.slug, total]);
      ...
      <div className={`mt-3 flex items-center gap-4 rounded-[2rem] bg-white p-5 shadow-sm ring-1 ${room.ring}`}>
        ...
        {phase === "explore" && (
          <div className="ml-auto hidden shrink-0 text-right sm:block">
            <div className="font-fun text-2xl font-700 text-slate-800">
              {solvedIds.length}/{total}
            </div>
            <div className="font-fun text-xs font-600 text-slate-400">solved</div>
          </div>
        )}
      </div>
      ...
      {phase === "intro" && (
        <div className="mt-4 rounded-[2rem] bg-white p-8 text-center shadow-sm ring-1 ring-amber-100">
          ...
          <p className="mt-3 font-fun text-sm font-600 text-slate-400">
            {total} objects to solve · ages {room.ageRange}
          </p>
          <button
            onClick={() => setPhase("explore")}
            className="mt-6 rounded-full bg-coral px-8 py-3 font-fun text-lg font-700 text-white shadow-lg transition hover:scale-105"
          >
            Enter the room 🚪
          </button>
        </div>
      ...
      {phase === "explore" && (
        <>
          ...
          <div className={`relative mt-4 h-[460px] overflow-hidden rounded-[2rem] shadow-sm ring-1 ${room.ring} sm:h-[520px]`}>
            {/* Themed scenery: wall texture, floor, and mood decor. */}
            ...
            <div className="absolute inset-x-0 top-0 flex justify-center p-3">
              <div className="rounded-full bg-white/85 px-4 py-1.5 font-fun text-xs font-700 text-slate-600 shadow-sm backdrop-blur">
                {allSolved ? "🔓 All done — head to the door!" : "Tap an object to solve its puzzle 🔍"}
              </div>
            </div>
            ...
            {/* Character — moves diagonally to its target (x and y both animate). */}
            <div
            ...
            >
              <span
                ...
                style={{
                  transform: `scaleX(${facingLeft ? -1 : 1}) rotate(${walkingTo ? "-6deg" : "0deg"})`,
                }}
              >
                {room.character}
              </span>
      ...
      {openStation && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <button
            ...
            onClick={() => !justSolved && setOpenId(null)}
            className="absolute inset-0 bg-slate-900/50 backdrop-blur-sm"
          />
          <div className="relative z-10 w-full max-w-lg rounded-[2rem] bg-white p-6 shadow-2xl ring-1 ring-amber-100 max-h-[90vh] overflow-y-auto">
            ...
            {justSolved && (
              <div className="mt-6 rounded-2xl bg-mint/15 p-5 text-center ring-1 ring-mint/30">
                ...
                >
                  {solvedIds.length === total ? "Back to the room 🚪" : "Keep exploring 🔍"}
                </button>
              </div>
            )}
          </div>
...
function SceneBackdrop({ room }: { room: EscapeRoom }) {
  return (
    <>
      ...
      {/* Decorative scenery (purely cosmetic, behind the objects). */}
      {room.decor.map((d, i) => (
        <span
          key={i}
          aria-hidden
